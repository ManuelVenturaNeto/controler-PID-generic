...
class PIDUI(tk.Tk):
    ...
    def _build_top_controls(self):
        ...
        # Linha 1
        row1 = ttk.Frame(frm); row1.grid(row=0, column=0, columnspan=5, sticky="ew")
        ...
        # Linha 2
        row2 = ttk.Frame(frm); row2.grid(row=1, column=0, columnspan=5, sticky="ew")
        ...
        self.cmb_modo = ttk.Combobox(row2, textvariable=self.var_modo,
    ...
    def _on_modo_change(self, *args):
        ...
        if new_mode != self._last_modo and new_mode == "manual":
            ...
            if self.running:
                self.running = False
                try: self.btn_pause.config(text="Retomar")
    ...
    def _prompt_manual_m(self):
        try: suggested = float(self.ctrl.vm)
        except Exception: suggested = 0.0
        lim = abs(self.ctrl.anti_reset_windap) or 1.0

        win = tk.Toplevel(self)
        win.title("Valor manual de m"); win.transient(self); win.grab_set()
        ...
        ent.grid(row=1, column=0, padx=(10,6), pady=4); ent.focus_set()

        def bump(v):
            try: var_val.set(round(float(var_val.get()) + v, 6))
            except Exception: pass
        ...
        def apply_and_close():
            try: val = float(var_val.get())
            except Exception: val = suggested
            if val >  lim: val =  lim
            if val < -lim: val = -lim
            ...
            # (sem gráfico de vm; apenas redesenha a tela)
            self.canvas.draw_idle()

            self._changing_mode = True
            try: self.var_modo.set("automatico")
            finally: self._changing_mode = False
            self._last_modo = "automatico"

            if self._was_running_before_manual:
                try: K_limit = int(self.var_K.get())
                except Exception: K_limit = self.ctrl.K
                if self.step_idx < K_limit:
                    self.running = True; self.started = True
                    try: self.btn_pause.config(text="Pausar")
                    except Exception: pass
                    self._tick()
                else:
                    try: self.btn_pause.config(text="Aumente K p/ retomar")
                    except Exception: pass
            win.destroy()

        def cancel_and_close(): win.destroy()

        btns = ttk.Frame(win); btns.grid(row=2, column=0, columnspan=3, pady=(8,10))
    ...
    def _build_plot(self):
        ...
        self.ax_ck.grid(True)
        self.ax_ck.set_autoscale_on(True)
    ...
    def start(self):
        ...
        if self.step_idx >= K_limit:
            try: self.btn_pause.config(text="Aumente K p/ iniciar")
            except Exception: pass
            return
        self.started = True
        self.running = True
    ...
    def toggle_pause(self):
        ...
        if not self.running and not self.started:
            try: self.btn_pause.config(text="Use Iniciar")
            except Exception: pass
            return
    ...
    def stop(self): self.toggle_pause()
    ...
    def reset(self):
        ...
        self.ctrl = GenericControler(defs)  # pv inicial vem daqui; depois não é mais sobrescrito
    ...
    def _apply_defs_to_ctrl(self):
        ...
        self.ctrl.kp = d["kp"]; self.ctrl.ki = d["ki"]; self.ctrl.kd = d["kd"]
        self.ctrl.K  = d["K"];  self.ctrl.mode = d["modo"]
        self.ctrl.action = ui2ctrl_action(d["acao"])
        self.ctrl.sp = d["sp"]; self.ctrl.ts = d["ts"]
    ...
    def _tick(self):
        ...
        # Atualiza a série de ck
        y_ck = float(out.get("ck", self.ctrl.vm))
        self.xdata.append(self.step_idx)
        self.ydata_ck.append(y_ck)
